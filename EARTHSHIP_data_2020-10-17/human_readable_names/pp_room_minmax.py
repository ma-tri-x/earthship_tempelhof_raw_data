...
def main():
    title = "Wohnzimmer Raumtemperatur taegliche und woechentliche MinMax-Temp"
    
    pP = PostProcess()
    
    input1 = "Wohnzimmer_temperatur_Luft_raw.dat"
    intermediate_file = "Wohnzimmer_temperatur_dailyWeekly_intermediate"
    output1 = "Wohnzimmer_temperatur_Luft_daily.dat"
    output2 = "Wohnzimmer_temperatur_Luft_weekly.dat"
    
    daily = 24*60*60
    weekly = 7*daily
    zero_time = 18*60*60 + 38*60 + 51
    ...
    if os.path.isfile("{}".format(output2)):
        print("Already finished processing daily to gnuplot readable file. If you want to re-process, rm {}".format(output2))
    else:    
